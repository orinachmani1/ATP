...
public class SimpleMazeGenerator extends AMazeGenerator
{
    ...
    public Maze generate(int rows, int cols) {
       ...
       // Position startPos=new Position(0,0);
        ...
        Position startPos = new Position(randomNum.nextInt(rows-1),randomNum.nextInt(cols-1));
        Position endPos = new Position(randomNum.nextInt(rows-1),randomNum.nextInt(cols-1));
        ...
        for (int i =0;i<rows;i++){
            for (int j =0;j<cols;j++){
                if (j==0&&i==0)
                    continue;
              if ( j==endPos.getRowIndex() || i==startPos.getColumnIndex() ){
                  simplemaze[i][j]=0;
              }
              else {
                  //simplemaze[i][j]=0;
